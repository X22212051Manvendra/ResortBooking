...
def homepage(request):
    all_location = Resorts.objects.values_list('location', 'id').distinct().order_by()
    if request.method == "POST":
        try:
            print(request.POST)
            resort = Resorts.objects.all().get(id=int(request.POST['search_location']))
            rr = []
            ...
            room = Rooms.objects.all().filter(resort=resort, capacity__gte=int(request.POST['capacity'])).exclude(
                id__in=rr)
            if len(room) == 0:
...
def panel(request):
    ...
    resort = Resorts.objects.values_list('location', 'id').distinct().order_by()

    response = render(request, 'staff/panel.html',
                      {'location': resort, 'reserved': reserved, 'rooms': rooms, 'total_rooms': total_rooms,
                       'available': available_rooms, 'unavailable': unavailable_rooms})
    return HttpResponse(response)
...
def edit_room(request):
    ...
    if request.method == 'POST' and request.user.is_staff:
        ...
        resort = Resorts.objects.all().get(id=int(request.POST['resort']))
...
def add_new_location(request):
    if request.method == "POST" and request.user.is_staff:
        ...
        county = request.POST['new_county']
        country = request.POST['new_country']

        resorts = Resorts.objects.all().filter(location=location, county=county)
        if resorts:
            messages.warning(request, "Sorry City at this Location already exist")
            return redirect("staffpanel")
        else:
            new_resort = Resorts()
            new_resort.owner = owner
            new_resort.location = location
            new_resort.state = county
            new_resort.country = country
            new_resort.save()
